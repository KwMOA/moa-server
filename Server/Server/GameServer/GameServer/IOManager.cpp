...
void IOManager::lobbyHandleSinglePlayReq(ConnectInfo* connectInfo, const char* body, int bodySize)
{
    GameLobbyPacket::SinglePlayReqPacket packet;
    ...
    memcpy(&sessionId, pBody, sizeof(SessionId_t));
    pBody += sizeof(SessionId_t);
    
    memcpy(&userInfo.userNo, pBody, sizeof(userInfo.userNo));
    pBody += sizeof(userInfo.userNo);
    
    int returnValue = GameServer::getInstance()->userMgr->addUnconnectedUser(&userInfo, &sessionId);
    ...
    User* user = GameServer::getInstance()->userMgr->getUnconnectedUserByUserNo(userInfo.userNo);
    
    user->setAiMode(true);
    //todo. set stage and tribe
    
    GameLobbyPacket::SinglePlayResPacket sendPacket;
    
    sendPacket.userNo = userInfo.userNo;
