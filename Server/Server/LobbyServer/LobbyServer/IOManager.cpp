...
void IOManager::gameReceiveData(ConnectInfo* connectInfo, const command_t cmd, const char* body, const int bodySize)
{
    switch(cmd)
    {
        ...
        case GameLobbyPacket::SINGLE_PLAY_RES:
            gameHandleSinglePlayRes(connectInfo, body, bodySize);
            break;
...
void IOManager::gameHandleSinglePlayRes(ConnectInfo* connectInfo, const char* body, int bodySize)
{
    GameLobbyPacket::SinglePlayResPacket packet;
    ...
    ClientLobbyPacket::SinglePlayResPacket sendPacket;
...
void IOManager::clientReceiveData(ConnectInfo* connectInfo, const uint32_t cmd, const char* body, const int bodySize)
{
    switch(cmd)
    {
        ...
        case ClientLobbyPacket::SINGLE_PLAY_REQ:
            clientHandleSinglePlayReq(connectInfo, body, bodySize);
            break;
...
void IOManager::clientHandleSinglePlayReq(ConnectInfo* connectInfo, const char* body, const int bodySize)
{
    ClientLobbyPacket::SinglePlayReqPacket packet;
    ...
    memcpy(&packet.stageNo, pBody, sizeof(packet.stageNo));
    pBody += sizeof(packet.stageNo);
    ...
    GameLobbyPacket::SinglePlayReqPacket sendPacket;
    memcpy(&sendPacket.sid, user->getSid(), sizeof(sendPacket.sid));
    sendPacket.userNo = user->getUserNo();
    sendPacket.tribe = packet.tribe;
    sendPacket.stageNo = packet.stageNo;
    ...
    memcpy(pSendBuffer, &sendPacket.stageNo, sizeof(sendPacket.stageNo));
    pSendBuffer += sizeof(sendPacket.stageNo);
    ...
    LobbyServer::getInstance()->network->sendPacket(gameServerInfo->connectInfo, (char*)&sendPacket, sizeof(sendPacket));
